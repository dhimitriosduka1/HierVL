...
class FrozenInTime(BaseModel):
    def __init__(
        ...
        if video_params["model"] == "SpaceTimeTransformer":
            ...
            drop_rate = video_params.get("drop_rate", 0.0)
            attn_drop_rate = video_params.get("attn_drop_rate", 0.0)
        ...
        if aggregation_params is not None and aggregation_params["do_aggregation"]:
            if aggregation_params["type"] not in ["self-attention", "average"]:
                raise NotImplementedError(
                    f"{aggregation_params['type']} invalid or not implemented"
                )
        ...
        if load_checkpoint not in ["", None]:
            ...
            newest_state_dict = state_dict_data_parallel_fix(
    ...
    def forward(
        ...
        if do_aggregation and batch_size is None:
            raise NotImplementedError(
                "If do_aggregation is activated, batch_size must be provided."
            )
        if video_only:
        ...
        text_data = data[
            "text"
        ]  # Either narration clip or summary clip, never stacked text
        video_data = data["video"]
        ...
        if do_aggregation:
            ...
            if text_stacked_embeddings is not None:
                text_parent_embeddings = self.aggregation(
                    text_stacked_embeddings, batch_size
                )
            else:
        ...
        if return_embeds and not do_aggregation:
            ...
            return (
                text_embeddings,
                video_embeddings,
                text_parent_embeddings,
                video_parent_embeddings,
                text_stacked_embeddings,
            )
            ...
            raise NotImplementedError
            return (
                sim_matrix(text_embeddings, video_embeddings),
                sim_matrix(text_parent_embeddings, video_parent_embeddings),
                text_stacked_embeddings,
            )
