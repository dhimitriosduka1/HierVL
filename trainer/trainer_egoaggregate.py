...
class Multi_Trainer_dist_EgoAgg(Multi_BaseTrainer_dist):
    ...
    def __init__(
        ...
        agg_data_loader=None,
        agg_valid_data_loader=None,
        lr_scheduler=None,
        len_epoch=None,
        writer=None,
        visualizer=None,
        ...
        self.do_hierarchical = (
            self.config["training_methods"]["hierarchical"]["intra-modal"]
            or self.config["training_methods"]["hierarchical"]["inter-modal"]
        )
        # self.writer = writer
    ...
    def _train_step(self, data, epoch, batch_idx, dl_idx, hierarchy="child"):
        ...
        if self.tokenizer is not None:
            ...
            if "aggregated_text" in data.keys():
                data["aggregated_text"] = self.tokenizer(
        ...
        if "aggregated_text" in data.keys():
            data["aggregated_text"] = {
                key: val.to(self.device) for key, val in data["aggregated_text"].items()
            }
        ...
        with torch.set_grad_enabled(True):
            ...
            if hierarchy == "parent":
                ...
                (
                    summary_embeds,
                    video_stacked_embeds,
                    text_embeds,
                    video_embeds,
                    text_stacked_embeds,
                ) = self.model(data, do_aggregation=True, batch_size=batch_size)
            else:
            ...
            if hierarchy == "parent" and self.do_hierarchical:
                ...
                video_stacked_embeds = video_stacked_embeds.view(
                    batch_size, -1, video_stacked_embeds.shape[1]
                )
                # Do video child feature sampling
                num_positives = self.config["training_methods"]["hierarchical"][
                    "num_positives"
                ]
                assert video_stacked_embeds.shape[1] > num_positives
                pos_indices = random.sample(
                    range(video_stacked_embeds.shape[1]), num_positives
                )
                ...
                text_stacked_embeds = text_stacked_embeds.view(
                    batch_size, -1, text_stacked_embeds.shape[1]
                )
                ...
                if True:
                    ...
                    pos_indices = random.sample(
                        range(text_stacked_embeds.shape[1]), num_positives
                    )
                    ...
                    text_aggregated_embeds = self.model.module.compute_text(
                        data["aggregated_text"]
                    )
                    text_aggregated_embeds = text_aggregated_embeds.view(
                        batch_size, -1, text_aggregated_embeds.shape[1]
                    )
                    num_positives = self.config["training_methods"]["hierarchical"][
                        "num_positives"
                    ]
                    ...
                    assert agg_v_embeds.shape[1] > num_positives
                    pos_indices = random.sample(
                        range(text_aggregated_embeds.shape[1]), num_positives
                    )
                    text_clip_embeds = text_aggregated_embeds[:, pos_indices, :]
                    agg_n_embeds = agg_n_embeds.view(
                        batch_size, -1, agg_n_embeds.shape[1]
                    )
                    agg_v_embeds = agg_v_embeds.view(
                        batch_size, -1, agg_v_embeds.shape[1]
                    )
                    n_clip_embeds = agg_n_embeds[:, pos_indices, :]
                    v_clip_embeds = agg_v_embeds[:, pos_indices, :]
            ...
            if hierarchy == "parent" and self.do_hierarchical:
                video_clip_embeds = self.allgather(
                    video_clip_embeds, self.n_gpu, self.args
                )
                video_clip_embeds = video_clip_embeds.view(
                    -1, video_clip_embeds.shape[-1]
                )
                text_clip_embeds = self.allgather(
                    text_clip_embeds, self.n_gpu, self.args
                )
                ...
                if (
                    ...
                    intra_video_loss = self.additional_losses[0](
                        sim_matrix(video_embeds, video_clip_embeds),
                        num_samples=num_positives_MILNCE,
                    )
                    intra_text_loss = self.additional_losses[1](
                        sim_matrix(text_embeds, text_clip_embeds),
                        num_samples=num_positives_MILNCE,
                    )
                ...
                if (
                    False
                ):  # self.config['training_methods']['hierarchical']['inter-modal']:
                    inter_parent_video_loss = self.additional_losses[2](
                        sim_matrix(video_embeds, text_clip_embeds),
                        num_samples=num_positives_MILNCE,
                    )
                    inter_parent_text_loss = self.additional_losses[3](
                        sim_matrix(video_clip_embeds, text_embeds),
                        num_samples=num_positives_MILNCE,
                    )
            ...
            if self.config["loss"]["type"] == "EgoNCE":
                ...
                if hierarchy == "parent" and not only_sa_no_summary_baseline:
                    if not only_video_with_summary_baseline:
                        clip_loss = self.loss(output1, sim_v, sim_n) + self.loss(
                            output2, sim_v, sim_n
                        )
                    else:
                        clip_loss = self.loss(
                            output2, sim_v, sim_n
                        )  # output1 is text and summary
                else:
            ...
            intra_loss_exists = (
                hierarchy == "parent"
                and self.do_hierarchical
                and total_intra_loss is not None
            )
            inter_loss_exists = (
                hierarchy == "parent"
                and self.do_hierarchical
                and total_inter_loss is not None
            )
        ...
        if self.writer is not None and self.args.rank == 0 and hierarchy == "child":
            ...
            self.writer.add_scalar(
                f"Loss_training/loss_{dl_idx}", loss.detach().item(), final_total
            )

        # if batch_idx % self.log_step == 0 and self.args.local_rank == 0:
        if (
            batch_idx % self.log_step == 0
        ...
        if hierarchy == "parent" and self.args.rank == 0:
            print(
                "[{}] Parent Train Epoch: {} dl{} {} Loss: {:.6f}".format(
                    datetime.now().strftime(r"%m%d_%H:%M:%S"),
                    epoch,
                    dl_idx,
                    "NA",
            ...
            self.writer.add_scalar(
                f"Agg_Loss_training/loss_{dl_idx}", loss.detach().item(), self.agg_count
            )
            self.agg_count += 1
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data_li in enumerate(zip(*self.data_loader)):
            ...
            for dl_idx, data in enumerate(data_li):
                ...
                if (
                    not catastrophic_forgetting_baseline
                ):  # In this baseline we only FT for summary
                ...
                if (batch_idx + 1) % self.agg_train_freq == 0:
                    try:
                        ...
                        self.agg_data_iter = iter(self.agg_data_loader[0])
                        agg_batch = next(self.agg_data_iter)
                    agg_loss = self._train_step(
                        agg_batch, epoch, 0, dl_idx, hierarchy="parent"
                    )
        ...
        if self.writer is not None and self.args.rank == 0:
            for dl_idx in range(len(self.agg_data_loader)):
                ...
                self.writer.add_scalar(
                    f"Agg_Loss_training/loss_total_{dl_idx}", tl, epoch - 1
                )
    ...
    def _valid_epoch_per_dataloader(
        self, epoch, data_loader, group_list=None, task=None
    ):
        """
                        ...
                        for option_idx in range(data["video"].shape[0]):
                            ...
                            if task == "SummaryMCQ":
                                ...
                                if option_idx == data["correct"].item():
                                    ...
                                    correct_ordered_video = data["video"][
                                        data["correct"].item()
                                    ]
                                    data_option["video"] = correct_ordered_video[
                                        torch.randperm(correct_ordered_video.shape[0]),
                                        :,
                                        :,
                                        :,
                                        :,
                                    ]  # After removing B and #options, the shape is [num_clips, num_frames, C, H, W]
                            else:
                                raise NotImplementedError
                            text_embed_option, _, _, vid_embed_option, _ = self.model(
                                data_option, do_aggregation=True, batch_size=1
                            )
    ...
    def _valid_epoch(self, epoch):
        ...
        Note:
            The validation metrics in log must have the key 'val_metrics'.
        """
        summarymcq_dict = self._valid_epoch_per_dataloader(
            epoch,
            self.agg_valid_data_loader,
            group_list=["SummaryMCQ"],
            task="SummaryMCQ",
        )
        shufflemcq_dict = self._valid_epoch_per_dataloader(
            epoch,
            self.agg_valid_data_loader,
            group_list=["ShuffleMCQ"],
            task="ShuffleMCQ",
        )
