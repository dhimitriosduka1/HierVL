...
class MultiInstanceRetrieval(TextVideoDataset):
    def _load_metadata(self):
        ...
        if self.split == "train":
            path_relevancy = os.path.join(
                self.meta_dir,
                "relevancy/caption_relevancy_EPIC_100_retrieval_train.pkl",
            )
        elif self.split in ["val", "test"]:
            path_relevancy = os.path.join(
                self.meta_dir, "relevancy/caption_relevancy_EPIC_100_retrieval_test.pkl"
            )
    ...
    def _get_caption(self, idx, sample):
        ...
        if self.split == "train":
            with open(
                os.path.join(
                    self.meta_dir, "relevancy/caption_train/{}.pkl".format(idx)
                ),
                "rb",
            ) as f_local:
