...
class MultiDistTextVideoDataLoader(MultiDistBaseDataLoaderExplicitSplit):
    def __init__(
        ...
        if collate_fn == "my_custom_collate" and (
            split == "train" or dataset_name == "HowTo100M_VC"
        ):  # For this dataset we want custom collate for both train and val
        ...
        dataset = dataset_loader(
            ...
            reader,
            neg_param,
            anticipation_param=anticipation_param,
            tau_a=tau_a,
            tau_o=tau_o,
        )
        super().__init__(
