...
class EgoAggregation(TextVideoDataset):
    def _load_metadata(self):
        ...
        if self.split == "train":
            ...
            if load_all_once:
                with open(
                    os.path.join(self.meta_dir, "summary_clips_hierarchy_full.json")
                ) as f:
    ...
    def _get_stacked_caption(self, sample, index=-1, num_text_samples=16):
        ...
        if len(datum["clip_text"]) == 0:
            ...
            for idx in range(num_text_samples):
                caption_summary, noun_vec_summary, verb_vec_summary = self._get_caption(
                    sample
                )
        ...
        try:
            ...
            indices = sorted(
                random.sample(range(0, len(datum["clip_text"])), num_text_samples)
            )
        except Exception as e:
    ...
    def _get_stacked_frames(self, sample, num_clips=16):
        ...
        # clip_quantum = 16.0
        clip_duration = min(
            8.0, sample["clip_end"] - sample["clip_start"]
        )  # TODO: Move to config
        start_timestamps = [
            sample["clip_start"]
            + (i / ((num_clips - 1) * 1.0))
            * (sample["clip_end"] - sample["clip_start"] - clip_duration - 1.0)
    ...
    def _get_train_item(self, item):
        ...
        # Text aggregation
        try:
            aggregated_caption, aggregated_noun_vec, aggregated_verb_vec = (
                self._get_stacked_caption(sample, index=item)
            )
        except Exception as e:
        ...
        if self.neg_param:
            print(
                "Not implemented. Summaries should sample negatives from the batch itself."
            )
            raise ValueError
