    ...
    def _load_metadata(self):
        ...
        if self.split == "train":
            ...
            if append_summary_baseline:
                ...
                self.summary_metadata = pd.read_csv(
                    os.path.join(self.meta_dir, summary_target_splits_fp),
                    sep="\t",
                    error_bad_lines=False,
                )
                print("Assigning summary duration to one of the clips...")
                for summary_idx in range(len(self.summary_metadata)):
                    self.summary_metadata.loc[summary_idx, "clip_start"] = (
                        random.uniform(
                            0.0,
                            self.summary_metadata.iloc[summary_idx]["clip_start"] - 4.0,
                        )
                    )
                    self.summary_metadata.loc[summary_idx, "clip_end"] = (
                        self.summary_metadata.iloc[summary_idx]["clip_start"]
                ...
                self.metadata = pd.concat(
                    [self.metadata, self.summary_metadata], ignore_index=True
                )
