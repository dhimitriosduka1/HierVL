...
def init_dataloaders(
    config, module_data, data_loader_type="data_loader"
):  # data_loader_type can be one of ["data_loader", "aggregate_data_loader"]
    """
    ...
    if "type" in config[data_loader_type] and "args" in config[data_loader_type]:
        ...
        config[data_loader_type]["args"] = replace_nested_dict_item(
            config[data_loader_type]["args"], "split", "val"
        )
        config[data_loader_type]["args"] = replace_nested_dict_item(
            config[data_loader_type]["args"], "batch_size", 1
        )
        valid_data_loader = [config.initialize(data_loader_type, module_data)]
    elif isinstance(config[data_loader_type], list):
        data_loader = [
            config.initialize(data_loader_type, module_data, index=idx)
            for idx in range(len(config[data_loader_type]))
        ]
        new_cfg_li = []
...
def main_worker(gpu, ngpus_per_node, args, config):  # TODO: Take config as input
    ...
    agg_data_loader, agg_valid_data_loader = init_dataloaders(
        config, module_data, data_loader_type="aggregate_data_loader"
    )
    if args.rank == 0:
        ...
        print(
            "Agg Val dataset: ",
            [x.n_samples for x in agg_valid_data_loader],
            " samples",
        )
    ...
    intra_modal_video_loss = (
        config.initialize(name="hierarchical_loss", module=module_loss)
        if config["training_methods"]["hierarchical"]["intra-modal"]
        else None
    )
    intra_modal_text_loss = (
        config.initialize(name="hierarchical_loss", module=module_loss)
        if config["training_methods"]["hierarchical"]["intra-modal"]
        else None
    )
    inter_parent_video_loss = (
        config.initialize(name="hierarchical_loss", module=module_loss)
        if config["training_methods"]["hierarchical"]["inter-modal"]
        else None
    )
    inter_parent_text_loss = (
        config.initialize(name="hierarchical_loss", module=module_loss)
        if config["training_methods"]["hierarchical"]["inter-modal"]
        else None
    )
    ...
    trainer = Multi_Trainer_dist_EgoAgg(
        args,
        model,
        loss,
        metrics,
